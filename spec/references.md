...
states plainly that it performs no sanitization. The same position is taken here, though not on
their authority: Svelte's client adopts whatever the server wrote for a raw block and replaces it
with the unsanitized value on the first update, so sanitizing on the server would buy nothing.
