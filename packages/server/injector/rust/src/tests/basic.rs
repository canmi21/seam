/* packages/server/injector/rust/src/tests/basic.rs */
...
fn text_slot_escapes_html() {
  let html =
    inject_no_script("<p><!--seam:msg--></p>", &json!({"msg": "<script>alert(\"xss\")</script>"}));
...
#[test]
fn attr_slot() {
  let html = inject_no_script("<!--seam:cls:attr:class--><div>hi</div>", &json!({"cls": "active"}));
...
fn cond_falsy_zero() {
  let html =
    inject_no_script("<!--seam:if:count--><p>has</p><!--seam:endif:count-->", &json!({"count": 0}));
  assert_eq!(html, "");
}
