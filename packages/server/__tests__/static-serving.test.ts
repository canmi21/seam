/* packages/server/__tests__/static-serving.test.ts */
...
describe("static asset serving", () => {
  ...
  it("serves CSS files with correct content type", async () => {
    const handler = makeHandler();
    const res = await req(handler, "GET", "/seam/assets/style-xyz.css");
    expect(res.status).toBe(200);
  ...
  it("serves nested files", async () => {
    const handler = makeHandler();
    const res = await req(handler, "GET", "/seam/assets/sub/nested.js");
    expect(res.status).toBe(200);
  ...
  it("returns 404 for missing assets", async () => {
    const handler = makeHandler();
    const res = await req(handler, "GET", "/seam/assets/nope.js");
  ...
  it("rejects directory traversal via URL normalization", async () => {
    ...
    const handler = makeHandler();
    const res = await req(handler, "GET", "/seam/assets/../../../etc/passwd");
    expect(res.status).toBe(404);
  });
  ...
  it("rejects path with encoded traversal attempt", async () => {
    ...
    const res = await handler({
      method: "GET",
      url: "http://localhost/seam/assets/..%2F..%2Fetc/passwd",
      body: () => Promise.reject(new Error("no body")),
    });
