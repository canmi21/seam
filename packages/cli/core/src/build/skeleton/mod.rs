...
mod tests {
  ...
  fn full_pipeline_snapshot() {
    ...
    let doc = wrap_document(&template, &["app.css".into()], &["app.js".into()], false);
  ...
  fn attribute_and_text_mixed_pipeline() {
    ...
    let doc = wrap_document(&result, &[], &[], false);
  ...
  fn float_hoisted_metadata_pipeline() {
    ...
    let doc = wrap_document(&slotted, &["style.css".into()], &["app.js".into()], false);
    assert!(doc.starts_with("<!DOCTYPE html>"));
  ...
  fn float_metadata_with_boolean_axis() {
    ...
    let doc = wrap_document(&template, &[], &[], false);
    let head = doc.split("</head>").next().unwrap();
    assert!(!head.contains("<!--seam:"), "no seam markers in <head>");
  }
}
