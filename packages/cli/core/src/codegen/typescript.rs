...
pub fn generate_typescript(manifest: &Manifest, rpc_hashes: Option<&RpcHashMap>) -> Result<String> {
  let mut out = String::new();
  out.push_str("// Auto-generated by seam. Do not edit.\n");

  if rpc_hashes.is_some() {
    out.push_str("import { createClient, configureRpcMap } from \"@canmi/seam-client\";\n");
  } else {
  ...
  // Emit hash map constant when obfuscating
  if let Some(map) = rpc_hashes {
    out.push_str("const RPC_HASH_MAP: Record<string, string> = {\n");
    for (name, hash) in &map.procedures {
      out.push_str(&format!("  \"{name}\": \"{hash}\",\n"));
    }
    out.push_str(&format!("  \"_batch\": \"{}\",\n", map.batch));
    out.push_str("};\nconfigureRpcMap(RPC_HASH_MAP);\n\n");
  }
...
mod tests {
  ...
  fn full_manifest_render() {
    ...
    let code = generate_typescript(&manifest, None).unwrap();
  ...
  fn full_manifest_render_with_hashes() {
    ...
    let code = generate_typescript(&manifest, Some(&hash_map)).unwrap();
