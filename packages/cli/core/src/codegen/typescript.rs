...
pub fn generate_typescript(manifest: &Manifest) -> Result<String> {
  ...
  out.push_str("import type { SeamClient, SeamClientError, Unsubscribe } from \"@canmi/seam-client\";\n\n");
  ...
  for (name, schema) in &manifest.procedures {
    ...
    if is_subscription {
      ...
      factory_lines.push(format!(
        "    {name}: (input, onData, onError) => client.subscribe(\"{name}\", input, onData as (data: unknown) => void, onError),"
      ));
    } else {
