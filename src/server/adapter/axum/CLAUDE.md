...
| `handler/` | Directory: mod.rs (AppState, build_router), rpc.rs, subscribe.rs, page.rs, channel.rs, projection.rs, stream.rs, upload.rs, sse_lifecycle.rs; page handler calls `seam_server::execute_derives` for derive execution, injects `__loaders` metadata, uses `inject_no_script()`; subscribe.rs passes `SubscriptionParams` with `last_event_id` from `Last-Event-ID` header; SSE events carry incrementing `id` |
