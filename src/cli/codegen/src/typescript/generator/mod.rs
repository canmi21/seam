...
fn generate_imports(has_stream: bool, emit_query_hooks: bool) -> String {
...
///
/// When `emit_query_hooks` is true, the output includes typed hook re-exports
/// bound to `SeamProcedureMeta` via TS instantiation expressions.
/// Enable only when the project depends on `@canmi/seam-query-react`.
pub fn generate_typescript(
	manifest: &Manifest,
	rpc_hashes: Option<&RpcHashMap>,
	data_id: &str,
	emit_query_hooks: bool,
) -> Result<String> {
	...
	out.push_str(&generate_imports(has_stream, emit_query_hooks));
