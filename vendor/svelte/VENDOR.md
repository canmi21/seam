...
**`_config.js` is taken but not run as written.** Each one is upstream's module importing
`{ test }` from the harness above, which is not here; `pkgs/suite` substitutes an identity for
that import and reads the object. What it reads out is `props`, and the four fields that say a
sample is not ours to run: `skip`, `mode`, `error` and `load_compiled`.

## What is checked

`mise run suite`, which is not part of `verify`. It compiles all 2388 samples and reports how
many are byte-identical to Svelte's own render, how many compiled and differ, how many were
refused and how many upstream skips itself. At the pinned tag it reports 1014 identical and 38
more that agree on an empty body, 115 differing, 662 refused and 558 skipped, with one sample
failing inside the oracle rather than inside either side. It takes eleven seconds.
`spec/suite.md` holds the reading of those numbers and the denominator they are against.
